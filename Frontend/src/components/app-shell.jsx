...
export default function AppShell({ children }) {
  ...
  return <div className="next-shell">
    ...
    <aside className={`next-sidebar ${menu ? 'is-open' : ''}`}>
      ...
      <div className="next-sidebar-bottom"><div className="next-help"><span className="connection-orbit"><i /></span><div><small>LIVE WORKSPACE</small><strong>{data.database}</strong><p>Changes sync automatically for your team.</p></div></div><div className="next-user"><span>{user.name?.slice(0, 2).toUpperCase()}</span><div><strong>{user.name}</strong><small>{roleLabel[user.role]}</small></div><button onClick={logout} aria-label="Sign out"><LogOut size={15} /></button></div></div>
    </aside>
