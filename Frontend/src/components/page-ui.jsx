...
export function PageIntro({ eyebrow, title, description, action }) {
  return <section className="role-hero"><span className="hero-glow" /><div className="hero-copy"><small>{eyebrow}</small><h2>{title}</h2><p>{description}</p></div>{action && <div className="hero-actions">{action}</div>}</section>;
}
...
export function Empty({ icon: Icon, title, text }) { return <div className="next-empty"><span><Icon size={22} /></span><strong>{title}</strong><p>{text}</p></div>; }
